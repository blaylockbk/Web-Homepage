...
# Open HRRR and get surface temp (or other varialbe)
grbs = pygrib.open(DIR+'hrrr/hrrr.t'+hour+'z.wrfsfcf00.grib2')
temp = grbs.select(name="Orography")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
lat, lon = grbs.select(name="Orography")[0].latlons()
land_sea = grbs.select(name="Land-sea mask")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
...
# Open HRRRx and get surface temp (or other variable)
grbsX = pygrib.open(DIR+'hrrrX/hrrrX.t'+hour+'z.wrfsfcf00.grib2')
tempX = grbsX.select(name="Orography")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
land_seaX = grbsX.select(name="Land-sea mask")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
...
if domain == 'GSL':
   m = draw_GSL_map()
    ...
    lon, lat = m(lon,lat)
# make sea a low number to make it blue
temp[land_sea==0]=-99
tempX[land_seaX==0]=-99
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, temp, cmap=terrain_cmap_256(), vmax=temp.max(), vmin=temp.min())
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, tempX, cmap=terrain_cmap_256(), vmax=temp.max(), vmin=temp.min())
plt.colorbar(orientation='horizontal', shrink=.9, pad=.03)
plt.title('hrrrX')
...
plt.suptitle(yesterday+" h"+hour+'z   Terrain and Water Mask')
