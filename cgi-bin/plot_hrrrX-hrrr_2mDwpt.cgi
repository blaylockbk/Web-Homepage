...
temp = grbs.select(name="2 metre dewpoint temperature")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]] - 273.15
...
tempX = grbsX.select(name="2 metre dewpoint temperature")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]] - 273.15
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, temp, cmap='BrBG', vmax=temp.max(), vmin=temp.min())
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, tempX, cmap='BrBG', vmax=temp.max(), vmin=temp.min())
plt.colorbar(orientation='horizontal', shrink=.9, pad=.03)
plt.title('hrrrX')
...
plt.suptitle(yesterday+" h"+hour+'z   2-m Dewpoint')
