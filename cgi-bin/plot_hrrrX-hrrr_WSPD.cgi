...
u = grbs.select(name="10 metre U wind component")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
v = grbs.select(name="10 metre V wind component")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
temp = wind_uv_to_spd(u,v)
lat, lon = grbs.select(name="2 metre dewpoint temperature")[0].latlons()
...
uX = grbsX.select(name="10 metre U wind component")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
vX = grbsX.select(name="10 metre V wind component")[0].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
tempX = wind_uv_to_spd(uX, vX)
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, temp, cmap='gist_stern_r', vmax=temp.max(), vmin=temp.min())
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, tempX, cmap='gist_stern_r', vmax=temp.max(), vmin=temp.min())
plt.colorbar(orientation='horizontal', shrink=.9, pad=.03)
plt.title('hrrrX')
...
plt.suptitle(yesterday+" h"+hour+'z   10-m Wind Speed')
