...
temp = grbs.select(name=other)[-1].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
lat, lon = grbs.select(name=other)[-1].latlons()
...
tempX = grbsX.select(name=other)[-1].values[subset[domain][0]:subset[domain][1],subset[domain][2]:subset[domain][3]]
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, temp, cmap='jet', vmax=temp.max(), vmin=temp.min())
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, tempX, cmap='jet', vmax=temp.max(), vmin=temp.min())
plt.colorbar(orientation='horizontal', shrink=.9, pad=.03)
plt.title('hrrrX')
...
m.drawcoastlines()
m.drawcountries()
m.pcolormesh(lon, lat, tempX-temp, cmap='bwr', vmax=(tempX-temp).max(), vmin=0-(tempX-temp).max())
plt.colorbar(orientation='horizontal', shrink=.9, pad=.03, extend='both')
plt.title('hrrrX - hrrr')

timer = datetime.now()-timer1
plt.suptitle(yesterday+" h"+hour+'z   '+other+'    PlotTimer:'+str(timer) )
